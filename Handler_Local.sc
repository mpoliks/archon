	...
	*new {
	...
	lushPlayer { // short flurries of events
		...
		Pkill(
			...
			\dur, Pseq([
				Pwhite(0.001, 0.02 * dur, (dur + 16).asInteger),
				Pwhite(1.4, 6.8, (dur + 2).asInteger)
			], dense),
	...
	crankPlayer { // semi-periodic assemblages of events
		...
		Pkill(
			...
			\cutoff, Env(
				[cent / 2, rrand(cent / 2, cent * 2), cent / 2],
				[rrand(2, 10), rrand(2, 40)],
				\sin),
	...
	glissPlayer { // rate envelope
		...
		Pkill(
			...
			\rate, Env(
				[rrand(0.7, 1.4), rrand(0.5, 1.4)],
				[rrand(4, 14)],
				\sin),

			\cutoff, Env(
				[bright * 1000, bright * 2000, bright * 500],
				[rrand(0.1, 4), rrand(4, 8)],
				\sin),
	...
	bubblePlayer { // granular synthesis engine corresponding to input frequency
		...
		rates = [2.0, 4.0],
		bright = args.at(\avgrolloff).linlin(500, 12000, 0, 2, clip: \minmax),
		freq = rrand(2, (dense - 8)),
		altfreq = rrand(2, (dense - 8)),
		buffno = rrand(0, 10),
		atk = 0.1;
		...
		Pkill(
			...
			\freq, Env(
				[freq, altfreq, freq],
				[rrand(3, 14), rrand(3, 14)],
				\sin),

			\dur, Pwhite(1, (dense / 3), inf),
			...
			\cutoff, Pwhite(4000, 16000),

			\pan, Pwhite(-1, 1),

			\amp, Pwhite(0.01, 0.04),
